...
void DeuteronSpectra() {
  ...
  ResultFile.mkdir("AntiDeuteron");
	...
	// DEUTERON
  TH1F hPrimaryFraction("hPrimaryFraction",";p_{T} (GeV/c);Primary fraction",5,bins);
  TH1F hSecondaryFraction("hPrimaryFraction",";p_{T} (GeV/c);Primary fraction",5,bins);
  ...
  ResultFile.cd();
	...
	for (int iCent = 0; iCent < 3; ++iCent)
	{
		...
		for (int iPt = 0; iPt < 13; ++iPt)
		{
			...
			TH1F *data = (TH1F*)f->Get(Form("Signal/fSignal%i_%i",iCent,iPt));
    ...
    ll2.AddEntry(rawSpectraD[i],titles[i].Data(),"lp");
		spectraCanvas->cd();
		rawSpectraD[i]->Draw((i==0) ? "e" : "esame");
  ...
  delete spectraCanvas;
	...
	for (int iCent = 0; iCent < 3; ++iCent)
	{
		...
		{
			TCanvas *cv = new TCanvas(Form("Cent%i_Pt_%i",iCent,iPt),Form("Cent%i_Pt_%i",iCent,iPt));
			TH1F *data = (TH1F*)f->Get(Form("Signal/fSignalAD%i_%i",iCent,iPt));
			data->SetMarkerStyle(20);
			data->SetMarkerSize(0.5);
			data->SetMarkerColor(kBlue);
			TF1 *tpl;
    ...
    llAD.AddEntry(rawSpectraAD[i],titles[i].Data(),"lp");
		spectraCanvasAD->cd();
		rawSpectraAD[i]->Draw((i==0) ? "e" : "esame");
