...
void DeutSelector::SlaveBegin(TTree * /*tree*/)
{
  ...
  for (int i = 0; i < 13; ++i) {
    fBins[i+1] = bins[i+1];
    fSignal[i] = new TH1D(Form("fSignal%i",i),Form("%4.1f #leq p_{T} #let %4.1f ;(m - m_{PDG})^{2};Entries",fBins[i],fBins[i+1]),600,-3.,3.);
...
void DeutSelector::Terminate()
{
  ...
  for (int i = 0; i < 13; ++i) {
    fSignal[i] = dynamic_cast<TH1D*>(GetOutputList()->FindObject(Form("fSignal%i",i)));
    if (fSignal[i]) {
      fSignal[i]->Write();
    }
  }
